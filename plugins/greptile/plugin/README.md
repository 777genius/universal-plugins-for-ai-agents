...
`greptile` is a real portable MCP plugin in this catalog.
...
It installs the official Greptile MCP server via HTTP and projects it into five targets from one authored source.
...
```bash
../plugin-kit-ai/bin/plugin-kit-ai normalize ./plugins/greptile
../plugin-kit-ai/bin/plugin-kit-ai generate ./plugins/greptile
```
...
`greptile` is a portable MCP package, so `codex-package` is the correct target.

## Why Remote HTTP

This plugin uses remote MCP transport with API-key auth in the header.

- it maps directly to Greptile’s hosted MCP endpoint
- API auth is controlled by environment variables for each target runtime
- it keeps the plugin surface intentionally focused on config-first setup
