...
```bash
cat src/plugin.yaml
cat src/mcp/servers.yaml
```

`src/plugin.yaml` defines shared metadata and target list.
`src/mcp/servers.yaml` defines remote MCP transport and GitHub auth header placeholder.
